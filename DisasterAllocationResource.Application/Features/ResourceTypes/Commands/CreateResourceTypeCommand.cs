...
namespace DisasterAllocationResource.Application.Features.ResourceTypes.Commands
{
    ...
    public class CreateResourceTypeCommandHandler(IResourceRepository resourceTypeRepo) : CommandHandler<CreateResourceTypeCommand>
    {
        public override async Task ExecuteAsync(CreateResourceTypeCommand command, CancellationToken ct)
        {
            var existResourceType = await resourceTypeRepo.GetByIdAsync(command.ResourceId, ct);
